...
async fn get_fullcalendar_events(period: web::Query<FullCalendarPeriod>) -> HttpResponse {
    ...
    let psql_url: &str = &handlers::utils::get_psql_url();
    let pool  = match handlers::utils::get_pool(psql_url).await {
        Ok(p) => p,
        Err(e) => return handlers::utils::internal_server_error(4001, &e.to_string()),
    };
    let events: Vec<FullCalendarEvent> = match sqlx::query_as(
        ...
        .fetch_all(&pool).await {
            Ok(item) => item,
            Err(e) => return handlers::utils::internal_server_error(4002, &e.to_string()),
        };
